...
export function getLastSync() {
  try {
    return JSON.parse(_ls().getItem(LAST_SYNC_KEY) || 'null')
  } catch {
    return null
  }
}
...
function setLastSync(info) {
  _ls().setItem(LAST_SYNC_KEY, JSON.stringify(info))
}
...
export async function pushToSupabase() {
  ...
  _ls().setItem('medconsult_last_backup', String(Date.now()))
}
...
function getMark(key) {
  return Number(_ls().getItem(key) || 0)
}
function setMark(key, ts) {
  _ls().setItem(key, String(ts))
}
