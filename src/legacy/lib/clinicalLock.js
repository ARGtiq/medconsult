...
//   в _ls(). Кто-то, укравший файлы устройства, всё ещё должен
...
export function isLockEnabled() {
  return _ls().getItem(LOCK_FLAG_KEY) === '1'
}
...
export function hasPin() {
  return !!_ls().getItem(PIN_WRAP_KEY)
}
...
export function readClinicalSync() {
  ...
  try {
    const raw = _ls().getItem(CLINICAL_KEY)
...
export function writeClinicalSync(data) {
  ...
  if (isLockEnabled()) {
    ...
    encryptWithPassword(data, sessionPassword).then((enc) => {
      _ls().setItem(CLINICAL_KEY, JSON.stringify({ __encrypted: true, ...enc }))
    })
  } else {
    _ls().setItem(CLINICAL_KEY, JSON.stringify(data))
  }
}
...
function rememberForSession(password) {
  ...
  try {
    _ss().setItem(SESSION_PASSWORD_KEY, password)
  } catch {
...
export async function trySessionUnlock() {
  ...
  let stored
  try {
    stored = _ss().getItem(SESSION_PASSWORD_KEY)
  } catch {
    return false
  }
...
export function disableLock() {
  ...
  _ls().removeItem(LOCK_FLAG_KEY)
  _ls().removeItem(PIN_WRAP_KEY)
  sessionPassword = null
  try {
    _ss().removeItem(SESSION_PASSWORD_KEY)
  } catch {
    // ignore
  }
  _ls().setItem(CLINICAL_KEY, JSON.stringify(current))
}
...
export async function enablePin(pin, password) {
  ...
  _ls().setItem(PIN_WRAP_KEY, JSON.stringify(enc))
}
...
export async function unlockWithPin(pin) {
  const raw = _ls().getItem(PIN_WRAP_KEY)
