...
// Supabase), они хранятся в _ls(). .env по-прежнему поддерживается как
...
export function getSupabaseConfig() {
  const url = _ls().getItem(URL_KEY) || import.meta.env.VITE_SUPABASE_URL || ''
  const anonKey = _ls().getItem(ANON_KEY) || import.meta.env.VITE_SUPABASE_ANON_KEY || ''
  return { url, anonKey }
}

export function setSupabaseConfig(url, anonKey) {
  _ls().setItem(URL_KEY, url.trim())
  _ls().setItem(ANON_KEY, anonKey.trim())
  cachedClient = null // пересоздать клиент при следующем обращении
}

export function clearSupabaseConfig() {
  _ls().removeItem(URL_KEY)
  _ls().removeItem(ANON_KEY)
  cachedClient = null
}
