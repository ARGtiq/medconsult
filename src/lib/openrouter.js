...
export async function extractGuidelineInfo(instructionText) {
  const raw = await callAI(
    'Ты извлекаешь структурированные данные из текста клинических рекомендаций для врача-уролога. Отвечай СТРОГО валидным JSON без markdown, без ```, без преамбулы. Формат: {"mkb10Codes": "коды через запятую, напр. N40, N40.1", "title": "краткое название состояния", "definition": "определение в 1-2 предложения", "diagnosisFormulation": "шаблон корректной формулировки диагноза для протокола, напр. Доброкачественная гиперплазия предстательной железы, стадия компенсации", "diagnostics": "что нужно для подтверждения диагноза, через запятую", "firstLine": "терапия первой линии кратко, через запятую названия препаратов/подходов", "secondLine": "терапия второй линии / когда направлять дальше", "redFlags": "тревожные признаки, требующие направления к специалисту/госпитализации", "source": "название документа-источника", "sourceYear": "год публикации, если указан, иначе пустая строка"}. Если что-то не найдено в тексте — пустая строка.',
    instructionText.slice(0, 12000)
  )
  const cleaned = raw.replace(/```json|```/g, '').trim()
  try {
